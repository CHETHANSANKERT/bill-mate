...
Future<Uint8List> generateSalesPdf(List<Map<String, dynamic>> selectedSales, {bool includeSummary = true}) async {
  ...
  for (var sale in selectedSales) {
    ...
    final Map<String, dynamic>? store = await DatabaseHelper().getStoresById(storeId) ?? null;
    pdf.addPage(
      pw.Page(
        ...
        build: (context) {
          return pw.Column(
            ...
            children: [
              ...
              textKeyValue('Date', DateTime.now().cddmmyyyy),
              ...
              pw.Row(
                mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                children: [
                  store?['address'] != null ? textKeyValue('Address', store?['address']) : pw.SizedBox.shrink(),
                  store?['mobileNum'] != null ? textKeyValue('Mobile Number',  store?['mobileNum'] ?? '') : pw.SizedBox.shrink(),
                ]
              ),
